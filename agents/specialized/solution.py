...
def solution_agent(client_info: str, reason: str, history: str) -> str:
    ...
    try:
        kb_results = agent.tool.retrieve(
            ...
            retrieveFilter={
            "startsWith": {"key": "approach", "value": "SOLUTIONS"},
            }
        )
    ...
    print(f"[DEBUG] RAG content for solution_agent: '{kb_text}'")
    prompt = PromptTemplates.solution_prompt(client_info, reason, history, kb_text=kb_text)      
    try:
